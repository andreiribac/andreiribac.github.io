...
(function ($) {
    ...
    var defaultConfig = {
        num: 3, // the number of images to display
        maxWidth: 250,   // max width of the active image
        maxHeight: 150, // min height of the active image
    ...
    function initStyle($container) {
        ...
        $container.find('.left').css({
            ...
            left: 0,
            top: 0
            // 'z-index': 0 + $container.data('totalNum') + 1
        });
        ...
        $container.find('ul li').each(function (index, element) {
            ...
            if (i !== undefined) {
                scales = Math.pow(config.scale, Math.abs(i - halfShowNum));
                zIndex = 0 + (i > halfShowNum ? (config.num - 1 - i) : i);
                $(element).css({
                    display: 'block',
                    position: 'absolute',
                    'z-index': zIndex,
